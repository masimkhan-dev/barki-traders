...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        ...
        VALUES
            (NEW.voucher_no, 'purchase', NEW.purchase_date, v_inv_id, NULL, NEW.total_amount, 0, 'Inventory Purchase', NEW.created_by),
    ...
    IF TG_OP = 'UPDATE' THEN
        ...
        v_value_changed :=
            OLD.rate_per_unit IS DISTINCT FROM NEW.rate_per_unit;
        ...
        ELSE
            ...
            IF v_value_changed AND COALESCE(v_qty, 0) < OLD.quantity THEN
                RAISE EXCEPTION 'PURCHASE EDIT BLOCKED: rate/amount edit requires % L available, but current stock is % L. Some stock has already been sold.', OLD.quantity, COALESCE(v_qty, 0);
            END IF;

            IF v_stock_changed OR v_value_changed THEN
                v_new_qty := COALESCE(v_qty, 0) + v_delta;
                v_new_value := (
                    COALESCE(v_qty, 0) * COALESCE(v_cost, 0)
                ) - (OLD.quantity * OLD.rate_per_unit) + (NEW.quantity * NEW.rate_per_unit);
