-- =================================================================
-- MIGRATION: 20260114000014_fix_voucher_type_enum.sql
-- PURPOSE: Fix ENUM casting for voucher_type
-- VERIFIER: Google DeepMind
-- =================================================================

-- 1. Ensure 'money_movement' is a valid value in the ENUM
-- We check if the type exists and alter it if necessary
DO $$
BEGIN
    ALTER TYPE voucher_type ADD VALUE IF NOT EXISTS 'money_movement';
EXCEPTION
    WHEN duplicate_object THEN null; -- Ignore if exists
    WHEN OTHERS THEN RAISE NOTICE 'Type voucher_type might not exist or be an enum';
END $$;

-- 2. Update Function with Explicit Casting
...
DECLARE
    ...
    v_dr_slug TEXT;
    v_cr_slug TEXT;
    -- remove constant, cast directly in query
BEGIN
    -- 1. Validation
    ...
    IF p_from_type = 'customer' THEN
        ...
        IF v_to_ar > 0 THEN
            ...
            VALUES 
                (v_dr_acct, p_movement_date, v_to_ar, 0, p_narration, v_voucher, 'money_movement'::voucher_type, 'money_movement', p_from_party_id),
                (v_cr_acct, p_movement_date, 0, v_to_ar, p_narration, v_voucher, 'money_movement'::voucher_type, 'money_movement', p_from_party_id);
        END IF;
        ...
        IF v_to_adv > 0 THEN
            ...
            INSERT INTO ledger_entries (account_id, posting_date, debit_amount, credit_amount, narration, voucher_no, voucher_type, reference_type, reference_id)
            VALUES 
                (v_dr_acct, p_movement_date, v_to_adv, 0, p_narration || ' (Adv)', v_voucher, 'money_movement'::voucher_type, 'money_movement', p_from_party_id),
                (v_cr_acct, p_movement_date, 0, v_to_adv, p_narration || ' (Adv)', v_voucher, 'money_movement'::voucher_type, 'money_movement', p_from_party_id);
        END IF;
    ...
    ELSIF p_to_type = 'supplier' THEN
        ...
        IF v_to_ar > 0 THEN
            ...
            INSERT INTO ledger_entries (account_id, posting_date, debit_amount, credit_amount, narration, voucher_no, voucher_type, reference_type, reference_id)
            VALUES 
                (v_dr_acct, p_movement_date, v_to_ar, 0, p_narration, v_voucher, 'money_movement'::voucher_type, 'money_movement', p_to_party_id),
                (v_cr_acct, p_movement_date, 0, v_to_ar, p_narration, v_voucher, 'money_movement'::voucher_type, 'money_movement', p_to_party_id);
        END IF;
        ...
        IF v_to_adv > 0 THEN
            ...
            INSERT INTO ledger_entries (account_id, posting_date, debit_amount, credit_amount, narration, voucher_no, voucher_type, reference_type, reference_id)
            VALUES 
                (v_dr_acct, p_movement_date, v_to_adv, 0, p_narration || ' (Adv)', v_voucher, 'money_movement'::voucher_type, 'money_movement', p_to_party_id),
                (v_cr_acct, p_movement_date, 0, v_to_adv, p_narration || ' (Adv)', v_voucher, 'money_movement'::voucher_type, 'money_movement', p_to_party_id);
        END IF;
    ...
    ELSE
        ...
        VALUES 
            (v_dr_acct, p_movement_date, p_amount, 0, p_narration, v_voucher, 'money_movement'::voucher_type, 'money_movement'),
            (v_cr_acct, p_movement_date, 0, p_amount, p_narration, v_voucher, 'money_movement'::voucher_type, 'money_movement');
    END IF;
