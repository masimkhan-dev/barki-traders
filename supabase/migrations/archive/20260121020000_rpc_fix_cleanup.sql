-- FINAL REPAIR SCRIPT (V2): Fix All Missing RPCs and Navigation
BEGIN;

-- 1. DROP ALL OLD VERSIONS to avoid ambiguity
...
BEGIN
    ...
    WITH base_data AS (
        ...
        UNION ALL
        SELECT 
            CASE WHEN p.type = 'customer' THEN 'CUST' ELSE 'SUPP' END as code, p.name,
        ...
        FROM parties p WHERE p.is_active = true
    )
    SELECT b.code, b.name, b.opening, b.debit, b.credit, (b.opening + b.debit - b.credit) as closing
    FROM base_data b WHERE ABS(b.opening) > 0 OR ABS(b.debit) > 0 OR ABS(b.credit) > 0;
END;
$$;

-- 3. RECREATE: BALANCE SHEET
CREATE OR REPLACE FUNCTION get_balance_sheet(p_date DATE)
RETURNS TABLE (category TEXT, sub_category TEXT, account_name TEXT, balance NUMERIC) LANGUAGE plpgsql AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION get_party_statement(p_party_id UUID, p_start_date DATE DEFAULT '2000-01-01', p_end_date DATE DEFAULT '2099-12-31')
...
-- 5. RECREATE: PARTY PRODUCT SUMMARY (MISSING IN PREVIOUS SCRIPT)
...
RETURNS TABLE (
    id UUID, date DATE, voucher_no TEXT, party_name TEXT, description TEXT, paid NUMERIC, received NUMERIC, running_balance NUMERIC
) AS $$
BEGIN
    RETURN QUERY
