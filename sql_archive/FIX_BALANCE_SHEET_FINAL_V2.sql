-- FIX BALANCE SHEET PARTY BALANCE (USE MARKET POSITION LOGIC)
-- Purpose: Match Dashboard by including party opening balances.
...
BEGIN
    ...
    -- 2. Calculate Party Receivables (MATCHES get_market_position_report)
    SELECT COALESCE(SUM(receivable_balance), 0)
    INTO v_total_receivables
    FROM get_market_position_report(p_date);

    -- 3. Calculate Party Payables (MATCHES get_market_position_report)
    ...
    UNION ALL
    -- A2. Market Receivables (FROM MARKET POSITION FUNCTION)
    ...
    UNION ALL
    -- B2. Supplier Payables (FROM MARKET POSITION FUNCTION)
